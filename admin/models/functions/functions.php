...
function updateMember ( $memberID, $firstName, $middleName, $lastName, $gender, $dob, $membership_date, $classification, $department, $marital_status, $role, $employment_date, $home_phone, $work_phone, $moblie_phone, $personal_email, $work_email, $residential, $postal, $Image, $imageLocation, $imageDestination )
{
    ...
    $connection = mysqlconnect();
    $stmt = $connection->prepare( 'UPDATE members SET firstName=?, middleName=?, lastName=?, gender=?, dob=?, membership_date=?, classification=?, department=?, marital_status=?, role=?, employment_date=?, 
                                  home_phone=?, work_phone=?, mobile_phone=?, personal_email=?, work_email=?, residential_address=?, postal_address=?, image=? WHERE memberID=?' );
    $stmt->bind_param( 'sssissiiiisssssssssi', $firstName, $middleName, $lastName, $gender, $dob, $membership_date, $classification, $department, $marital_status, $role, $employment_date, $home_phone, $work_phone, $moblie_phone, $personal_email, $work_email, $residential, $postal, $Image, $memberID);
    $stmt->execute();
...
function updateMemberA ( $memberID, $firstName, $middleName, $lastName, $gender, $dob, $membership_date, $classification, $department, $marital_status, $role, $employment_date, $home_phone, $work_phone, $moblie_phone, $personal_email, $work_email, $residential, $postal )
{
    // Check if the file field is not empty
    ...
    $connection = mysqlconnect();
    $stmt = $connection->prepare( 'UPDATE members SET firstName=?, middleName=?, lastName=?, gender=?, dob=?, membership_date=?, classification=?, department=?, marital_status=?, role=?, employment_date=?, 
                                  home_phone=?, work_phone=?, mobile_phone=?, personal_email=?, work_email=?, residential_address=?, postal_address=? WHERE memberID=?' );
    $stmt->bind_param( 'sssissiiiissssssssi', $firstName, $middleName, $lastName, $gender, $dob, $membership_date, $classification, $department, $marital_status, $role, $employment_date, $home_phone, $work_phone, $moblie_phone, $personal_email, $work_email, $residential, $postal, $memberID);
    $stmt->execute();
...
*/
function createMember ( $firstName, $middleName, $lastName, $gender, $dob, $membership_date, $classification, $department, $marital_status, $role, $employment_date, $home_phone, $work_phone, $moblie_phone, $personal_email, $work_email, $residential, $postal, $Image, $imageLocation, $imageDestination, $defaultPass )
{
    // Check if the file field is not empty
    if ( !empty( $Image ) )
    {
    ...
    if( !$stmt->affected_rows == 0 ) {
        ...
        $password = password_hash($defaultPass, PASSWORD_BCRYPT, $options);
        ...
        if(registerUser($last_id,$work_email, $password,$creationDate,$lastLogin,$loginCount)){
            ...
            if (inputSubscribers($cycleNo, $startDate, $endDate, $cycleDuration, $subscriberID, $subscriberName, $amountComputed, $subStatus)){
                ...
                $subject ='RE: DEFAULT PASSWORD FOR PAYSWITCHADWANE';

                // Details
                $message='An account on PAYSWITCHADWANE has been created for you with this email address and a default password as "'.$defaultPass.'" Kindly change your password when you logon';

                $mail_from= 'hr@payswitch.com.gh';
                $name='HR MANAGER';

                $transport = Swift_SmtpTransport::newInstance('ssl://smtp.gmail.com', 465)
                    ->setUsername('graybeard86@gmail.com')// Your Gmail Username
                    ->setPassword('ILoveSuzzy123@'); // Your Gmail Password
                ...
                $message = Swift_Message::newInstance($subject)
                    ->setFrom(array($mail_from => $name))// can be $_POST['email'] etc...
                    ->setTo(array($work_email => 'Undisclosed Party'))// your email / multiple supported.
...
function cycleInfo($cycleID){
    $info = array();
    $connection = mysqlconnect();
    $stmt = $connection->prepare(" SELECT c.cycleStart, c.cycleEnd, a.noOfDays FROM cycles c LEFT JOIN cycleaccounts a ON c.cycleID=a.cycleNo WHERE c.cycleID=? " );
    $stmt->bind_param('i',$cycleID);
    $stmt -> execute();
    $stmt -> bind_result( $info[0],$info[1],$info[2] );
    $stmt->fetch();
    return $info;
